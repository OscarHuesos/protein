...
#mlp = KerasClassifier(model, epochs=300, batch_size=32, verbose=0)
...
print(MCC_mlp)
#pyplot.figure()
#sn.heatmap(confusion_matrix(y_test , y_mlp), annot=True,values_format = '.3f')
#pyplot.show()

#fpr_rf, tpr_rf, thresholds_rf = roc_curve(y_test, y_pred_rf)
#auc_rf = auc(fpr_rf, tpr_rf)
#fpr3n,tpr3n, _ = roc_curve(y_test,y_svm)
#roc_auc3 = auc(fpr3n,tpr3n)
#print("y_test")
#print(y_test)
#print("y_log2")
#print(y_log2)
...
#    print ("ii es ")
#    print (i)
#    fpr1[i], tpr1[i], _ = roc_curve(y1_test, y_score1)
...
#fpr4n, tpr4n, thresholds = roc_curve(y_test,y_log2)
#roc_auc4= auc(fpr4n,tpr4n)

#fp_mlp, tpr_mlp, _ = roc_curve(y_test,y_mlp)
#roc_auc_mlp = auc(fp_mlp,tpr_mlp)
#plot_confusion_matrix(y_mlp, X_test, y_test)
#print("mlp salida")
#print(y_mlp)
#fpr1n = dict()
#tpr1n = dict()
#roc_auc1 = dict()
#fp_naive, tpr_naive, th1 = roc_curve(y_test, prueba_naive )
#roc_auc_naive = auc(fp_naive, tpr_naive)
#for i in range(nc):

#fpr1n, tpr1n, tr1 = roc_curve(y_test, y_mlp)
#    fpr1n[i], tpr1n[i], _ = roc_curve(y_test[:, i], y_mlp[:, i])
#roc_auc1 = auc(fpr1n, tpr1n)
...
#lw = 2
#plt.figure()
#plt.plot(fpr5n, tpr5n, label='naive')
#plt.plot(fp_forest, tpr_forest, label='Forest')
#plt.plot(fp_smv, tpr_svm, label= 'smv')
#plt.plot(fp_mlp, tpr_mlp, label='mlp')
#plt.plot(fpr1n, tpr1n , label= 'test')
...
#prueba_naive = y_naive[:, 1]
#auc1=roc_auc_score(y_test,prueba_naive)
#print('AUC: %.2f' % auc1)
#plt.plot(fpr[2], tpr[2], color='darkorange',lw=lw, label='sdgfdsd (area = %0.2f)' % roc_auc[2])
#plt.plot([0, 1], [0, 1], color='navy', lw=lw, linestyle='--')
#plt.xlim([0.0, 1.0])
#plt.ylim([0.0, 1.05])
#plt.xlabel('Tasa de Falsos Positivos')
#plt.ylabel('Tasa de Verdaderos Positivos')
#plt.title('Curvas ROC de dos clasificadores SVMs')
#plt.legend(loc="lower right")
#plt.grid(True)
#print("Plot ROC")
#plt.show()
#plt.plot(fpr1[2], tpr1[2], color='red',lw=lw, label='bsbxvx (area = %0.2f)' % roc_auc1[2])
#plt.plot([0, 1], [0, 1], color='navy', lw=lw, linestyle='--')
#plt.legend(loc="lower right")

#mlp_plot= plot_roc_curve(mlp, X_test, y_test)
...
ax = pyplot.gca()
#pyplot.plot(fpr_log, tpr_log,label='Logistic Regression Classifier (area = {:.3f})'.format(auc_log))
#pyplot.plot(fpr_forest, tpr_forest,label='Random Forest Classifier (area = {:.3f})'.format(auc_forest))
#pyplot.plot(fpr_svm, tpr_svm,label='Support Vector Classifier (area = {:.3f})'.format(auc_svm))
...
#naive_plot= plot_roc_curve(naive_result ,x_test, y_test,ax=ax)
#forest_plot.figure_.suptitle("ROC curve comparison")
#pyplot.legend(loc=0)
pyplot.show()
